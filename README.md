...
제품 페이지는 정적 페이지로 생성하였다. 정적 페이지가 검색 엔진 최적화에 좋기 때문이다. 또한 페이지의 오픈그래프나 트위터카드에 제품 사진과 정보를 포함하기 위해서는 정적 페이지여야 한다.

정적 페이지 생성은 **Next.js**의 `getStaticProps`를 사용하였다.

정적 페이지는 빌드 시 생성되지만 업데이트가 필요할 때 마다 매 번 빌드할 수는 없으므로 **`revalidate`** 를 통해 페이지의 재생성을 요청하는 방법을 사용할 수 있다. `getStaticProps` 의 리턴에 `revalidate` 트리거 타임을 명시해주면 페이지의 요청이 발생한 뒤 명시한 시간이 지난 뒤 페이지가 자동으로 업데이트된다.
...
다만 현재 배포에 사용 중인 Netlify가 해당 기능을 지원하지 않는다는 사실을 뒤늦게 알게 되어 적용하지는 못했다. 
...
글 작성을 끝내고 호스팅 서비스를 Vercel로 변경 후 다시 적용해볼까 생각 중이다.
